...
function initDom() {
    ...
    window.addEventListener('resize', function() {
        // 이전 방향 저장
        var prevAspect = aspect;
        var wasVertical = prevAspect < 1;
        // 현재 화면 비율 계산
        aspect = window.innerWidth / window.innerHeight;
        var isVertical = aspect < 1;
    ...
    document.getElementById('restart-from-pause-btn').addEventListener('click', function() {
        // 다시 시작하기 확인
        pauseMenuDialog.close();
...
function renderGrid() {
    ...
    var displayRows = isVertical ? COLS : ROWS;
    var displayCols = isVertical ? ROWS : COLS;
    // 그리드 컨테이너 스타일 설정
    gameGridElement.style.gridTemplateRows = "repeat(".concat(displayRows, ", ").concat(CELL_SIZE, "px)");
    gameGridElement.style.gridTemplateColumns = "repeat(".concat(displayCols, ", ").concat(CELL_SIZE, "px)");
    // 세로 또는 가로 모드에 따라 그리드 렌더링
    for(var i = 0; i < displayRows; i++){
        for(var j = 0; j < displayCols; j++){
            // 원본 그리드 좌표 계산
            var originalRow = isVertical ? ROWS - 1 - j : i; // -90도 회전을 위해 행 인덱스 반전
            var originalCol = isVertical ? i : j; // -90도 회전 좌표 변환
            var cell = grid[originalRow][originalCol];
            ...
            // 셀 요소 캐싱 (성능 최적화)
            var cellId = "".concat(originalRow, "-").concat(originalCol);
...
function getCellCoordinatesFromPosition(x, y) {
    ...
    var displayCol = Math.floor(validRelativeX / (CELL_SIZE + GRID_GAP));
    var displayRow = Math.floor(validRelativeY / (CELL_SIZE + GRID_GAP));
    // 유효한 범위로 제한
    ...
    if (relativeX < 0 || relativeY < 0 || relativeX > (CELL_SIZE + GRID_GAP) * displayCols || relativeY > (CELL_SIZE + GRID_GAP) * displayRows) {
...
function updateSelection(e) {
    ...
    var validRow = Math.max(0, Math.min(currentCell.row, ROWS - 1));
    var validCol = Math.max(0, Math.min(currentCell.col, COLS - 1));
    // 보정된 좌표로 셀 업데이트
    var validCell = {
        row: validRow,
        col: validCol
    ...
    if (!selectionEndCell || validCell.row !== selectionEndCell.row || validCell.col !== selectionEndCell.col) {
...
function startPhysicsAnimation(physics, onComplete) {
    ...
    function animate(timestamp) {
        ...
        if (elapsed >= physics.duration / 1000) {
            // 애니메이션 목록에서 제거
            var index = activeAnimations.indexOf(animationId);
            if (index !== -1) {
...
function updateTimer() {
    if (isPaused) return; // 일시 정지 상태일 때는 타이머 업데이트 중지
    timeLeft -= 0.1;
    // 타이머 표시 업데이트
    timeElement.textContent = timeLeft.toFixed(1);
...
// ... 기존 전역 변수들 ...
var isPaused = false; // 게임 일시 정지 상태
var pauseBtn = null; // 일시 정지 버튼
var pauseMenuDialog = null; // 일시 정지 메뉴 다이얼로그
var confirmationDialog = null; // 확인 다이얼로그
var pendingAction = null; // 확인 대기 중인 작업
var activeAnimations = []; // 활성 애니메이션 목록을 추적하기 위한 배열
// 전역 변수에 일시 정지 힌트 상태 변수 추가
// ... 기존 전역 변수들 ...
var pausedHintState = null; // 일시 정지 시 현재 힌트 상태 저장
// 게임 일시 정지 함수
function pauseGame() {
    ...
    clearInterval(gameTimer);
    // 현재 힌트 상태 저장
    if (isHintVisible && currentHint) {
        pausedHintState = {
            hint: _to_consumable_array(currentHint),
            remainingTime: HINT_DURATION // 정확한 남은 시간은 알 수 없으므로 기본값 사용
        };
        // 화면에서 힌트 제거 (상태는 유지)
        document.querySelectorAll('.cell.hint').forEach(function(cell) {
            cell.classList.remove('hint');
...
function resumeGame() {
    ...
    // 저장된 힌트 상태가 있으면 힌트 복원
    if (pausedHintState && timeLeft > 0) {
        // 현재 힌트 복원
        currentHint = _to_consumable_array(pausedHintState.hint);
        isHintVisible = true;
        // 힌트 표시 (일시 정지 해제 플래그로 호출)
        currentHint.forEach(function(cell) {
            var cellElement = document.querySelector('.cell[data-row="'.concat(cell.row, '"][data-col="').concat(cell.col, '"]'));
            if (cellElement && !cellElement.classList.contains('empty')) {
                cellElement.classList.add('hint');
        ...
        hintTimer = setTimeout(function() {
            return hideHint(true);
        }, pausedHintState.remainingTime);
        // 상태 초기화
        pausedHintState = null;
    } else if (!isHintVisible && timeLeft > 0) {
        hintTimer = setTimeout(showHint, HINT_DELAY);
    }
